...
_take_serialized_message(
  ...
  rmw_subscription_allocation_t * allocation)
{
  std::cout<<"begin shared take serialized message"<<std::endl;
  (void) allocation;
  *taken = false;
...
*/
      ...
      *taken = true;
  printf("begin ndn take\n");
  ndn::Interest interest(info->topic);
  printf("create interest\n");
  Subscriber subscriber(interest);
  subscriber.subscribe_to_topic(serialized_message,interest);
  return RMW_RET_OK;
}
